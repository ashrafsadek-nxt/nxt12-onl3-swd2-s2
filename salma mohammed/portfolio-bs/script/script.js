...
projects.forEach(project => {
  ...
  card.className ="d-flex flex-column flex-md-row align-items-center light-background rounded-4 p-5";
  card.style.gap = "30px"; 
card.style.minWidth = "100%";
card.style.minHeight = "400px";
  card.innerHTML = `
          <img src="${project.image}" alt="${project.name}" style="height:100%;" class="w-50 w-md-50">
          <div>
          <h2>${project.name}</h2>
...
skills.forEach(skill => {
  const card = document.createElement("div");
  card.className = "d-flex flex-column align-items-center col-12 col-md-6 col-lg-4";
  card.innerHTML = `
          <img class="ms-0 ms-lg-4" width="80px" src="${skill.image}" alt="${skill.name}">
          <p class="fs-5">${skill.name}</p>
        `;
